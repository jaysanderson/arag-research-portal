...
page (or a local run). Gates (`deno task check`, `build:web`) do not catch visual, layout, theme
or UX bugs. Before calling any UI work done, check with your own eyes:
- **Light mode AND dark mode** - theme-specific bugs are common (translucency bleed, contrast,
  seams where a translucent surface straddles two backgrounds).
...
- World-class, never "vibe coded". Considered typography, spacing, hierarchy, motion; real
  empty/loading/error states. Use the `rp-*` design tokens and component classes, never raw
  one-off colours.
- **Use the full screen on large displays.** Layouts scale UP on 2xl+ (27-inch, maximised
  windows), not only down to mobile. Avoid fixed centred `max-w-6xl` columns that strand half a
  large monitor; keep prose columns at a readable measure (~65-75ch) while using the extra width
  for grids, rails and viewers.
